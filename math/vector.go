...
func (v Vec2) Divide(f float64) Vec2 {
...
func MinCompsVec(v, w Vec3) Vec3 {
	return Vec3{math.Min(v.X, w.X), math.Min(v.Y, w.Y), math.Min(v.Z, w.Z)}
}

func MaxCompsVec(v, w Vec3) Vec3 {
	return Vec3{math.Max(v.X, w.X), math.Max(v.Y, w.Y), math.Max(v.Z, w.Z)}
}

func DotV3(v, w Vec3) float64 {
	return v.X*w.X + v.Y*w.Y + v.Z*w.Z
}

func AbsDotV3(v, w Vec3) float64 {
	return math.Abs(DotV3(v, w))
}

func CrossV3(v, w Vec3) Vec3 {
	return Vec3{v.Y*w.Z - v.Z*w.Y, v.Z*w.X - v.X*w.Z, v.X*w.Y - v.Y*w.X}
}

func MakeCoordSystem(v, w, u *Vec3) {
	if math.Abs(v.X) > math.Abs(v.Y) {
		sqrt := math.Sqrt(v.X*v.X + v.Z*v.Z)
		*w = Vec3{-v.Z, 0, v.X}.Divide(sqrt)
	} else {
		sqrt := math.Sqrt(v.Y*v.Y + v.Z*v.Z)
		*w = Vec3{0, v.Z, -v.Y}.Divide(sqrt)
	}
	*u = CrossV3(*v, *w)
}
